...
| Yazi config | `dot_config/yazi/` | `yazi.toml`, `keymap.toml`, `desired-plugins.txt`. Targets `~/.config/yazi` on both OSes — see below. Both the `ya pkg`-installed `plugins/` dir and `package.toml` itself are deliberately **not** tracked (see below); `desired-plugins.txt` is the real cross-machine plugin declaration instead — generated by `scripts/sync-yazi-plugins.sh`, not hand-edited. |
...
- **Yazi** has its own dedicated override, `YAZI_CONFIG_HOME` — set that (Windows user env
  ...
  reruns when `desired-plugins.txt` actually changes. Needs `.chezmoi.toml.tmpl`'s
  `[interpreters.sh]` mapping to run at all on Windows — chezmoi doesn't auto-detect
  shebangs there.
  **To add a plugin: `ya pkg add owner/repo` locally, then run `scripts/sync-yazi-plugins.sh`**
  — it extracts every `use = "..."` line out of the local `package.toml` and overwrites
  `desired-plugins.txt` with the result, so no plugin name ever gets typed into that file by
  hand; it's a mechanical mirror of whatever `package.toml` already says. Review the diff it
  prints and commit. This is still one manual step per machine, unavoidably — chezmoi only
  propagates state through committed source, there's no live sync between devices, so
  something has to turn "what I just installed here" into "what every machine should have."
