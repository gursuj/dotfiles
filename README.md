...
| WP-CLI / Local by Flywheel shell fix (the `.zshrc`/`.bashrc` sourcing lines) | `dot_zshrc.tmpl`, `dot_bashrc` | Sources `~/programming/local-wpcli/wp-shell-integration.{zsh,sh}` (that script itself lives outside chezmoi, in a separate standalone repo — see its own `HANDOFF.md`). In `dot_zshrc.tmpl` the line is placed *before* the `CLAUDECODE` skip-block, deliberately — Claude Code's own shells run `wp` in these site dirs too and need the fix, unlike the interactive-only keybind/completion/plugin setup that block skips. `dot_bashrc` is new and, for now, arch-only (`.chezmoiignore`) — this is the wp-cli fix only, not a general bashrc consolidation; see `TODO.md`'s bash-on-vps note. |
...
| Kanata config | `dot_config/kanata/kanata.kbd.tmpl` | Targets `~/.config/kanata/kanata.kbd` on both Windows and the arch box (`{{ if eq .chezmoi.os "windows" }}` gates the few things that differ, e.g. dropping `1` from defsrc to dodge a Windows-only WTQ hotkey race). Caps = layer key: tap Esc, hold = Fn layer (number row → F-keys, WASD → arrows). Caps+Grv force-releases stuck modifiers — manual workaround for a kanata bug (jtroo/kanata #423, #1553) where a modifier release can get lost on a keyboard-grab race. Replaces PowerToys KM, which was intermittently dropping the intercept on a cheap membrane keyboard (confirmed not rollover/ghosting first, via NKRO tester). Excluded entirely on the VPS via `.chezmoiignore` (no physical keyboard to remap there). On the arch box it autostarts via `dot_config/systemd/user/kanata.service` (also excluded on the VPS); on Windows it runs via elevated Task Scheduler (`AtLogOn`, 15s delay — Kanata fails silently if started too early) since Startup folder can't grant admin rights. |
