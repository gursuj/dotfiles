...
| OpenCode TUI plugin config | `dot_config/opencode/tui.jsonc` | Just `{ "plugin": ["./herdr-tui-session.js"] }` — small, stable wiring, not itself herdr-managed generated code (see below), so tracked like `settings.json` is for Claude Code. |
...
category as the yazi/herdr plugin pattern below, just for herdr's own agent-state reporting
