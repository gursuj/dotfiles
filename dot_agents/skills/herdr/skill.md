...
## diagnosing issues and bug reporting

the herdr server writes a structured log at:

```
C:\Users\User\AppData\Roaming\herdr\herdr-server.log   (Windows)
~/.local/share/herdr/herdr-server.log                  (Linux/macOS — typical)
```

read the tail when something fails — it usually has the exact error:

```powershell
Get-Content "C:\Users\User\AppData\Roaming\herdr\herdr-server.log" -Tail 40
```

**what to look for:**

- `ERROR` lines — these are the smoking gun. they include the subsystem, exact command, cwd, and the OS error code.
- `pane.spawn.start` / `pane.spawned` — confirms a pane launched (or didn't). if `.spawn.start` appears but `.spawned` is missing, the process died before it started.
- `api.request.start` / `api.request.complete` — confirms the CLI reached the server. if `.start` is missing, the socket wasn't reached at all.
- `tab.create` / `workspace.create` — confirms the action completed server-side.
- `pane.exit` with status `ExitStatus { code: 3221225786 }` — that's `STATUS_CONTROL_C_EXIT` on Windows, normal close.
- `PaneDied for unknown pane` — harmless warning about a pane that closed before the server tracked its death.

**for bug reports:** copy the relevant ERROR lines plus a few lines of context before them. the log includes the full command string, cwd, and OS error — that's usually enough for the maintainer to reproduce.

**known Windows-specific bug (prefix+e scrollback editor):** herdr tries to open scrollback content in `$EDITOR` using `/bin/sh -c "..."`, which doesn't exist on Windows. the log shows `CreateProcessW /bin/sh ... failed: The system cannot find the path specified. (os error 3)`. no config workaround exists — this requires herdr to use the configured `default_shell` on Windows.
